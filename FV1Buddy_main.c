...
int main(void)
{
  ...
  uint8_t tap = 1; //1 for tap tempo control, 0 for pot control
  ...
  uint16_t mstempo = 500; //Current tempo in ms
  ...
  ADC_Config();
    ...
    while (1)
    {
        ...
        if ((tap == 1 && abs(previoustimepot-pot) >= 15) || (timepresetactive == 1 && tap == 0 && abs(previoustimevalue-timevalue) >= 15) || (timepresetactive == 0 && tap == 0 && abs(previoustimevalue-timevalue) >= 1))//if pot move of more than 5%, changing to pot control
		{
        ...
        if (abs(previousdiv - divsw) > 50)		//if first time or if div toggle changed position : update div
		{
			...
			{
				if (divsw <= 50){divmult = 1;}	//fourth
			
				if (divsw > 50 && divsw < 230){divmult = 0.75;}	//dotted eighth
				
				if (divsw >= 230){divmult = 0.5;}	//eighth
			}
			
			if (debounce() == 1)	//if tap button pressed while changing 3 last div
			{
				if (divsw <= 50){divmult = 0.333333;}	//triplet
				
				if (divsw > 50 && divsw < 230){divmult = 0.25;}	//sixteenth
				
				if (divsw >= 230){divmult = 0.1666666;}	//sextuplet
